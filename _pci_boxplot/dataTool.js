 ...
 */
 numberUtil.quantile = function (ascArr, p) {
    var H = (ascArr.length - 1) * p + 1;
    // var H = ascArr.length * p + p;
    var h = Math.floor(H);
    var v = +ascArr[h - 1];
    var e = H - h;
    console.log('H: ' + H + ' , h: ' + h + ' , v:' + v + ' , e: ' + e);
...
function _default(rawData, opt) {
  ...
  // console.log('rowData: ' + JSON.stringify(rawData));
  for (var i = 0; i < rawData.length; i++) {
    axisData.push(i + '');
    var ascList = numberUtil.asc(rawData[i].slice());
    console.log('ascList: ' + JSON.stringify(ascList));
    ...
    // console.log('min: ' + min + ' , max: ' + max + ' , bound: ' + bound);
...
var prepareBoxplotData = _default;
